...
Now, the ideal propensity score, $\tilde{\phi}$ would summarize just the variation in $X_1$ and omit any variation in $Z$.  Instead, $Z$ should be summarized in it's own "randomization" axis.  Figure 3 depicts the same matched dataset, projected down onto each pair of axes ($\tilde{\phi} \times \Psi$, $IV \times \Psi$, and $IV \times \tilde{\phi}$).  The matchings shown were produced from Mahalanobis distance matching on all covariates except $Z$ with propensity and prognostic score calipers.  In figure 3A we see that the selected pairs are very close in terms of propensity and prognosis, but they tend to be more distant in terms of $Z$ (Figure 3B-C). This visualization may be an especially useful companion to study designs using a nearfar matching approach [@baiocchi2012near]. The randomization-assignment-control plot may also have analouges in the randomized experiment setting, wherein the propensity score axis is omitted, and the "instrument" axis is replaced by an axis summarizing compliance behavior (e.g. "dose").
